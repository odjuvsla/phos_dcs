...
//í°»R(W)í°… and Broadcast í°»Yes*í°… can be written to when unlock bit (register 0x0)is set.
...
namespace BCRegisterMap
{
 //Register name                      Addr.   range      Type       Allow Brcast    Description
  ...
  const unsigned long  HV_FB1        = 0x25;   // [15:0]   R(W)      Yes*           Compared outputs from DAC for CSP
                                               // [7:0]                             CSP 16  %GÅ‚Äì%@ CSP 23
                                               // [15:8]                            CSP 7 %GÅ‚Äì%@ CSP 0
                                               //                                   0: DAC not set/Wrong
                                               //                                   1: DAC set/Correct
  const unsigned long HV_FB2         = 0x26;   // [15:0]   R(W)      Yes*           Compared outputs from DAC for CSP
                                               //                                   [7:0] CSP 15 %GÅ‚Äì%@ CSP 8
                                               //                                   [15:8] CSP 24 %GÅ‚Äì%@ CSP 31
                                               //                                   0: DAC not set/Wrong
                                               //                                   1: DAC set/Correct
  ...
  const unsigned long ADC_DIFF_DIR   = 0x2A;   // [14:0]    R(W)      Yes*           Sets the expected direction of the current
                                               ...
                                               //                                   1: Current = Vcurrent %GÅ‚Äì%@ Vpreviou
  ...
  const unsigned long ADC_Min_Threshold[15] = {0x30, 
					       ...
					       0x3C, 
					       0x3D, 
					       0x3E}; // [14:0] RW   Yes            Min Threshold for the ADCs
                                                      //                            [15:11] Hamming code
  ...
  const unsigned long ADC_Max_Threshold[15] = {0x40,
					       ...
					       0x4C, 
					       0x4D, 
					       0x4E}; // [14:0] RW   Yes            Max Threshold for the ADCs
                                                      //                            [15:11] Hamming code
                                                      //                            [10] 0: Threshold for Voltage
                                                      //                            1: Threshold for Current
                                                      //                            [9:0]    Data value
...
*/
  ...
  const unsigned long TEMP1         = 0x50; //                                      Temperature for ADC IC13  0.25Å∞C   *  ADC counts
  ...
  const unsigned long TEMP2         = 0x55; //                                      Temperature for ADC IC15  0.25Å∞C   *  ADC counts
  ...
  const unsigned long TEMP3         = 0x5A; //                                      Temperature for ADC IC14  0.25Å∞C   *  ADC counts
